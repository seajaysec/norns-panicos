...
# These 64-bit .so are the replacements the launch-time 32-bit strip relies on.
...
# tree from sclang's class paths). These are the 64-bit copies the launch-time
# 32-bit strip keeps; mirrors ingenue's verified on-device heal.
